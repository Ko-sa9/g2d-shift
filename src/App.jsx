...
// 条件操作ハンドラ
  ...
  const executeAutoFill = async () => {
    ...
    try {
      ...
      【基本ルール】
      1. 既存データの維持と活用: 既にシフトが入っている箇所は絶対に変更しないでください。ただし、**「すでに手入力されているシフト」のメンバーとそのpowerScoreも、その日の必要人数とパワーバランスの計算に必ず含めて考慮してください**。
      2. AI除外: "excludeFromAi": true の職員は変更しない。
      3. 3クール制限: 職員の「maxCool3」は、3クールシフト（コード: A, F）の月間上限回数。これを超えないこと。
      4. パワーバランスの確保（超重要）:
         各職員には「powerScore（総合力）」が設定されています。各施設の1日あたりの合計powerScoreが、以下の基準を「絶対に下回らないように」シフトを組んでください。
      ...
      6. 必要人数の厳守: 各曜日・各シフトごとの目標人数は、提供データ内の「targetCounts」を必ず参照して満たすようにしてください。
  ...
  const handleAiSend = async () => {
    ...
    try {
      ...
      if (aiChatMode === 'create') {
        ...
        // ★修正：ワンクリック作成と同じ「基本ルール」と「適用条件」を組み込み、ユーザーの指示を最優先させるよう変更しました
        prompt = `
        【役割】
        あなたは熟練の勤務表管理者です。「ユーザーからの指示」を最優先にしつつ、以下の基本ルールと適用条件を厳密に守ってシフト表(JSON)を作成・修正してください。
        ...
        4. パワーバランスの確保: 各施設の1日あたりの合計powerScoreが以下を下回らないようにしてください。
           - 坂田: 合計 30点 以上
           - 君津: 合計 30点 以上
           - 木クリ: 合計 40点 以上
           ※じんクリとME室はパワーバランス考慮不要。
